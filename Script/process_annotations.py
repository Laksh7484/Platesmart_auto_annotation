...
def detect_with_plate_recognizer(img_bytes, task_id):
    ...
    try:
        # Add a small delay to avoid overwhelming the API when running concurrently
        time.sleep(0.1)

        # Prepare JSON payload upfront for fallback/retry
        ...
        # Try multipart form data approach (common for file uploads)
        files = {
        ...
        # If that fails, try with base64 JSON
        ...
        # Handle rate limiting with brief backoff and retry using JSON payload
...
def process_single_task(task, worker_id):
    ...
    try:
        ...
        if check_existing_annotations(task):
            print(f"⏭️ [Worker {worker_id}] Skipping task {task_id} - already has annotations")
        ...
        if not img_bytes:
            print(f"⚠️ [Worker {worker_id}] Could not fetch image for task {task_id}")
            return {"status": "error", "task_id": task_id, "reason": "image_fetch_failed"}

        img_array = cv2.imdecode(np.frombuffer(img_bytes, np.uint8), cv2.IMREAD_COLOR)
        if img_array is None:
            print(f"⚠️ [Worker {worker_id}] Could not decode image {img_url} for task {task_id}")
        ...
        if api_result:
            ...
            if results:
                ...
                print(f"ℹ️ [Worker {worker_id}] No objects detected in task {task_id}")
                return {"status": "success", "task_id": task_id, "reason": "no_objects"}
        else:
            print(f"⚠️ [Worker {worker_id}] Failed to get results from Plate Recognizer Snapshot SDK for task {task_id}")
            return {"status": "error", "task_id": task_id, "reason": "api_failed"}
            
    except KeyError as e:
        print(f"⚠️ [Worker {worker_id}] Missing data in task {task.get('id', 'unknown')}: {e}")
        return {"status": "error", "task_id": task.get('id', 'unknown'), "reason": f"missing_data: {e}"}
    except Exception as e:
        print(f"⚠️ [Worker {worker_id}] Error processing task {task.get('id', 'unknown')}: {e}")
...
def main():
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                ...
                print(f"📊 Progress: {total_completed}/{len(tasks)} ({progress_percent:.1f}%) "
                      f"(✅ {processed_tasks}, ⏭️ {skipped_tasks}, ❌ {error_tasks})")
    ...
    print(f"⏱️ Total processing time: {processing_elapsed:.2f} seconds")
    if total_done:
        print(f"⏱️ Throughput (all tasks): {total_done/processing_elapsed:.2f} tasks/sec")
    if processed_attempts:
        print(f"⏱️ Throughput (annotate attempts): {processed_attempts/processing_elapsed:.2f} tasks/sec")
